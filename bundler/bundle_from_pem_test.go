...
var pemTests = []pemTest{
	{
		// int-bundle.pem is the SHA-256 intermediate generated by gen_testdata.sh,
		// signed by the root in ca-bundle.pem.
		cert:               mustReadFile("testdata/int-bundle.pem"),
...
// mustReadFile reads a file and returns its contents.
// Used to load test fixtures whose bytes change when gen_testdata.sh is re-run.
// Panics if the file is missing so the failure is obvious at startup.
func mustReadFile(path string) []byte {
	b, err := os.ReadFile(path)
	if err != nil {
		panic("mustReadFile: " + err.Error())
	}
	return b
}
...
// selfSignedCert is a fresh self-signed cert generated by gen_testdata.sh.
// It has no issuer in ca-bundle.pem so bundling returns error 1100 (unknown authority).
var selfSignedCert = mustReadFile("testdata/self-signed.pem")
